...
void HashTable::print() // prints all the contents of the stack
{
	...
	{
		cout << obsoleteVariables.top().type << " " << obsoleteVariables.top().name << " " << obsoleteVariables.top().address << ", ";
		obsoleteVariables.pop();
	}
}

string HashTable::find(const string& searchString) // returns the string searched for if found otherwise returns "Not Found"
{
	int a = convertString(searchString);
	if (table[a]->search(searchString) != NULL)
		return table[a]->search(searchString)->getData().name;
	else
		return "Not Found";
}
